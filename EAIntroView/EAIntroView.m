...
- (void)buildUIWithFrame:(CGRect)frame {
    ...
    [self.experienceRightNow setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
}
...
{
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, PhoneHeight - self.pageControlY, self.frame.size.width, 20)];
    [self.pageControl setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
    ...
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width - 80, PhoneHeight-40, 80, self.pageControl.frame.size.height)];
    ...
    [self.skipButton addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchDown];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    self.experienceRightNow = [[UIButton alloc] initWithFrame:CGRectMake(98, PhoneHeight - 100, 124, 34)];
    [self.experienceRightNow setBackgroundImage:[UIImage imageNamed:@"experienceRightNow.png"] forState:UIControlStateNormal];
    [self.experienceRightNow setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
    [self.experienceRightNow addTarget:self action:@selector(experienceRightNowAction:) forControlEvents:UIControlEventTouchDown];
    [self addSubview:self.experienceRightNow];
    self.skipButton.alpha = 1;
    self.experienceRightNow.alpha = 0;
    if(self.currentPageIndex == pageViews.count-1)
    {
        [UIView animateWithDuration:0.5 animations:^{
            self.skipButton.alpha = 0;
            self.experienceRightNow.alpha = 1;
        } completion:nil];
...
-(void)experienceRightNowAction:(id)sender
{
    [self.delegate introDidFinish];
    [self hideWithFadeOutDuration:0.6];
}
...
-(void)setExperienceRightNow:(UIButton *)experienceRightNow
{
    [_experienceRightNow removeFromSuperview];
    _experienceRightNow = experienceRightNow;
    [_experienceRightNow addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_experienceRightNow];
}
