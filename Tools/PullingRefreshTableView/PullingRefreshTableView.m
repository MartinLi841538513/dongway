    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            if (!self.isAtTop) {    //footer
                _arrow.hidden = YES;
                _stateLabel.text = NSLocalizedString(@"没有更多数据了哦", @"");
            }
        }
    }
}
...
@synthesize bottomOnly = _bottomOnly;
...
-(void)setBottomOnly:(BOOL)bottomOnly
{
    _bottomOnly = bottomOnly;
    _headerView.hidden = _bottomOnly;
}
...
- (void)flashMessage:(NSString *)msg{
    ...
    if (_msgLabel == nil) {
        ...
        _msgLabel.backgroundColor = [UIColor grayColor];
