...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            ...
            searchButton.frame=CGRectMake(0,0,90,21);
            [searchButton setBackgroundImage:[UIImage imageNamed:@"allImages.png"] forState:UIControlStateNormal];
            [searchButton addTarget:self action:@selector(handleSeeAllTouch:) forControlEvents:UIControlEventTouchDown];
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [self arrangeThumbs];
    searchButton.frame=CGRectMake(0,0,54,21);
    [searchButton setBackgroundImage:[UIImage imageNamed:@"sureButton.png"] forState:UIControlStateNormal];
...
//    [self.navigationItem.rightBarButtonItem setTitle:@"确定"];
    ...
    {
        [_thumbsView setFrame:CGRectMake(0, self.navigationController.navigationBar.frame.origin.y+self.navigationController.navigationBar.frame.size.height, 320, self.view.frame.size.height)];
    }
